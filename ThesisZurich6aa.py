...
file = open("stdZurich5a", "r")
bcsd = file.readline()
file.close()

file = open("Zurichbclist5a", "rb")
betlist = pickle.load(file)
file.close()

file = open("Zurichbestbcvector5a", "rb")
vectorbc = pickle.load(file)
file.close()

f = open("ZurichGlist5a", "rb")
    ...
    while good:
        ...
        for i in range(nflip):
            try:
                ...
                gx.edges[list(primaryneighedges)[chaos[i]][1], list(primaryneighedges)[chaos[i]][0], 0].update(
                    attributes)
                gx.remove_edge(list(primaryneighedges)[chaos[i]][0], list(primaryneighedges)[chaos[i]][1])
                print(list(primaryneighedges)[chaos[i]][0], "        ", list(primaryneighedges)[chaos[i]][1])
                for u, v, d in gx.edges(data=True):
                    if d['name'] == attributes['name'] and (v, u) != (
                    list(primaryneighedges)[chaos[i]][1], list(primaryneighedges)[chaos[i]][0]) and d['oneway'] == True:
                        flipname.append((u, v))
    ...
    while good:
        ...
        for i in range(nflip):
            try:
                ...
                gx.edges[list(secondaryneighedges)[chaos[i]][1], list(secondaryneighedges)[chaos[i]][0], 0].update(
                    attributes)
                gx.remove_edge(list(secondaryneighedges)[chaos[i]][0], list(secondaryneighedges)[chaos[i]][1])
                print(list(secondaryneighedges)[chaos[i]][0], "        ", list(secondaryneighedges)[chaos[i]][1])
                for u, v, d in gx.edges(data=True):
                    if d['name'] == attributes['name'] and (v, u) != (
                    list(secondaryneighedges)[chaos[i]][1], list(secondaryneighedges)[chaos[i]][0]) and d['oneway'] == True:
                        flipname.append((u, v))
                        flippedname.append((v, u, d))
                        # print(flipname)
                gx.add_edges_from(flippedname)
    ...
    while good:
        ...
        for i in range(nflip):
            try:
                ...
                gx.edges[list(tertiaryneighedges)[chaos[i]][1], list(tertiaryneighedges)[chaos[i]][0], 0].update(
                    attributes)
                gx.remove_edge(list(tertiaryneighedges)[chaos[i]][0], list(tertiaryneighedges)[chaos[i]][1])
                print(list(tertiaryneighedges)[chaos[i]][0], "        ", list(tertiaryneighedges)[chaos[i]][1])
                for u, v, d in gx.edges(data=True):
                    if d['name'] == attributes['name'] and (v, u) != (
                    list(tertiaryneighedges)[chaos[i]][1], list(tertiaryneighedges)[chaos[i]][0]) and d['oneway'] == True:
                        flipname.append((u, v))
                        flippedname.append((v, u, d))
                        # print(flipname)
                gx.add_edges_from(flippedname)
...
file=open("bcfinalAA", "w")
file.write("%f\n" % bcfinal)
file.close()

file=open("bclistAA", "wb")
pickle.dump(betlist, file)
file.close()

file=open("GminimumAA", "wb")
